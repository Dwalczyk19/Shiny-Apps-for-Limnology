...
R2_base <- function(data) {
  ...
  lr_data1 <- data[which(data$Depth <= 2),]
...
ui <- navbarPage(
  strong("BIC - Calculating 1%, 10% and normal extinction coefficients (Kd)"), 
  #tabsetPanel(
  tabPanel("Instructions", 
           column(12, align = "left",
                  
                  h4(tags$b("Use this Shiny App to calculate extinction coefficients from BIC measurements. This app consists of 4 tabs: ")), 
                  br(),
                  br(),
                  strong("Offset Row (1): "), HTML("Upload your lake's BIC data file where specified. After loading in the data file a Depth plot will pop up showing the differences in Depth and Edz320m as the BIC instrument is lowered into the water. Using the plot find the index where the depth and Edz320 change systematically and type it into the text box. 
                                     This is the only required step in calculating extinction coefficients."), 
                  br(),
                  br(),
                  strong("UV Check (2): "), HTML("Check to see if there are linear relationships for each wavelength measured as depth rises. If there are fluctuations, the instrument might have gone under the boat or been covered by a shadow. Redo if necessary."), 
                  br(), 
                  br(),
                  strong("Regression Profiles (3): "), HTML("Find the most linear relationship of each wavelength (y) compared to depth (x). Base settings measure the best R2 value between .5 and 1m, except for PAR which is measured to max depth. If the base depth constraints are
                                      not optimal, you can highlight the most linear relationship to check and find the extinction coefficient."), 
                  br(), 
                  br(),
                  strong("Kd Calculated (4): "), HTML("Kd @ 1% and 10% irradiance are calculated for each wavelength and are plotted in a dataframe to be reviewed. Once satisfied the user can download the dataset.")
                  )
           ),
  tabPanel("(1) Offset Row",
     sidebarLayout(
       sidebarPanel(width = 10,
         fileInput("upload", label = "BIC File", multiple = FALSE,
                   accept = c("text/csv", 
                              ...
                              ".csv")), 
         numericInput("offset_idx", "Pick the row index where EDZ (r) and Depth (z) change systematically:", value = 0, width = "60%")
       ),  #where depth changes
       mainPanel(
  ...
  tabPanel("(3) Regression Profiles",
           verbatimTextOutput("Note_Regress"),
           hr(), 
           verbatimTextOutput("check"), #for testing only
           splitLayout(
...
server <- function(input, output, session) {
  ...
  output$p1_R2 <- renderPrint({
    ...
    if (is.null(check)) {
      ...
      x_min = 0
      x_max <- KD_arr[3]
      z305min(round(x_min,3))
      z305max(round(x_max,3))
      R2305(KD_arr[1])
      paste("R2:",KD_arr[1], ", Kd:", KD_arr[2], ", Depth-Range: [ 0 ,",KD_arr[3],"]")
    } else {
      x_min <- ifelse(check$xmin >= 0, check$xmin, 0) #just always make sure there are never negative values
      x_max <- check$xmax
  ...
  output$p2_R2 <- renderPrint({
    ...
    if (is.null(check)) {
      ...
      x_min <- 0
      x_max <- KD_arr[3]
      z320min(round(x_min,3))
      z320max(round(x_max,3))
      R2320(KD_arr[1])
      paste("R2:",KD_arr[1], ", Kd:", KD_arr[2], ", Depth-Range: [ 0 ,",KD_arr[3],"]")
    } else {
      x_min <- ifelse(check$xmin >= 0, check$xmin, 0)
      x_max <- check$xmax
      KD_arr <- R2(arr, x_min, x_max)
      KD320(KD_arr[2])
      z320min(round(x_min,3))
  ...
  output$p3_R2 <- renderPrint({
    ...
    if (is.null(check)) {
      ...
      x_min = 0
      x_max = KD_arr[3]
      z380min(round(x_min,3))
      z380max(round(x_max,3))
      R2380(KD_arr[1])
      paste("R2:",KD_arr[1], ", Kd:", KD_arr[2], ", Depth-Range: [ 0 ,",KD_arr[3],"]")
    } else {
      x_min <- ifelse(check$xmin >= 0, check$xmin, 0)
  ...
  output$p4_R2 <- renderPrint({
    ...
    if (is.null(check)) {
      ...
      KDPAR(KD_arr[2])
      x_min = 0
      x_max = round(arr[nrow(arr), 'Depth'],3) #can't retrieve from R2() as R2 only returns a vector with 2 indices
      zPARmin(round(x_min,3))
      zPARmax(round(x_max,3))
      R2PAR(KD_arr[1])
      paste("R2:",KD_arr[1], ", Kd:", KD_arr[2], ", Depth-Range: [ 0 ,",x_max,"]")
    } else {
      x_min <- ifelse(check$xmin >= 0, check$xmin, 0)
      x_max <- check$xmax
      KD_arr <- R2(arr, x_min, x_max)
  ...
  return_data <- reactive({
    validate(
      need(!is.null(KD305()), "Please enter an index in (1)")
    )
    KD <- c(KD305(), KD320(), KD380(), KDPAR())
    zmin <- c(z305min(), z320min(), z380min(), zPARmin())
    zmax <- c(z305max(), z320max(), z380max(), zPARmax())
    R2 <- c(R2305(), R2320(), R2380(), R2PAR())
    data.frame(wavelength = c("305", "320", "380", "PAR"), Kd = KD, 
               Zd10 = log(10) / KD, Zd1 = log(100) / KD, min_depth = zmin, 
               max_depth = zmax, R2 = R2)
  })
...
#uv check, see if shadows are present or someone waved their hand over, or BIC instrument goes underneath the boat
#number tabs, add depth ranges


#Clarify basic background & process flow, taken from excel template in instructions,
#Notes and reason on each tab
#Make slides showing use of excel format and really measuring the accuracy between excel template and app 